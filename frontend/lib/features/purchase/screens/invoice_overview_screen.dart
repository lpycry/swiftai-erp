...
class InvoiceOverviewScreen extends StatefulWidget {
  ...
  const InvoiceOverviewScreen({super.key, required this.authService, required this.purchaseService});
...
class _InvoiceOverviewScreenState extends State<InvoiceOverviewScreen> {
  ...
  Set<String> get _vendors => _pos.map((p) => p.vendorName.isNotEmpty ? p.vendorName : p.vendorCode).toSet();

  List<PurchaseOrderModel> get _filtered {
    var list = _pos;
    if (_vendorFilter != null && _vendorFilter!.isNotEmpty) {
      list = list.where((p) =>
        p.vendorName.contains(_vendorFilter!) || p.vendorCode.contains(_vendorFilter!)).toList();
    }
    return list;
  }
        ...
        if (!_loading && _error == null)
          Container(
            ...
            child: Row(children: [
              ...
              Text('${filtered.length} PO(s) pending invoice',
                  style: TextStyle(fontWeight: FontWeight.w500, color: Colors.blue.shade800, fontSize: 13)),
              const Spacer(),
              Text(
                'Open: \$${PurchaseService.fmtAmount(filtered.fold<double>(0.0, (s, p) => s + _openAmount(p)))}',
                style: TextStyle(fontWeight: FontWeight.w600, color: Colors.orange.shade700, fontSize: 12),
              ),
            ]),
          ),
              ...
              children: [
                Padding(
                  ...
                  child: FilterChip(
                    ...
                    onSelected: (_) { setState(() => _vendorFilter = null); },
                    visualDensity: VisualDensity.compact,
                    selectedColor: Colors.blue.withValues(alpha: 0.15),
                  ),
                ),
                ..._vendors.map((v) => Padding(
                  padding: const EdgeInsets.only(right: 6),
                  child: FilterChip(
                    label: Text(v, style: const TextStyle(fontSize: 11), overflow: TextOverflow.ellipsis),
                    selected: _vendorFilter == v,
                    onSelected: (_) { setState(() => _vendorFilter = v); },
                    visualDensity: VisualDensity.compact,
                    selectedColor: Colors.blue.withValues(alpha: 0.15),
                  ),
                )),
        ...
        if (filtered.isNotEmpty)
          Container(
            ...
            child: Row(children: [
              ...
              const Expanded(flex: 1, child: Text('Date', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 10))),
              Expanded(flex: 1, child: Text('Open', textAlign: TextAlign.right,
                  style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 10))),
              Expanded(flex: 1, child: Text('Progress', textAlign: TextAlign.center,
                  style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 10))),
            ]),
          ),
              ...
              : _error != null
                  ...
                  : filtered.isEmpty
                      ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          Text('All POs invoiced!', style: TextStyle(fontSize: 16, color: Colors.grey.shade600)),
                          const SizedBox(height: 4),
                          Text('No POs pending invoice',
    ...
    return Container(
      ...
      child: InkWell(
        onTap: () async {
          ...
          if (mounted) {
            final created = await Navigator.push(context, MaterialPageRoute(
        ...
        child: Row(children: [
          ...
          Expanded(flex: 2, child: Text(po.vendorName.isNotEmpty ? po.vendorName : po.vendorCode,
              style: const TextStyle(fontSize: 11), overflow: TextOverflow.ellipsis)),
          Expanded(flex: 1, child: Text(_fmtDate(po.poDate),
              style: TextStyle(fontSize: 10, color: Colors.grey.shade600))),
          Expanded(flex: 1, child: Text('\$${PurchaseService.fmtAmount(openAmt)}', textAlign: TextAlign.right,
              style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
                  color: openAmt > 0 ? Colors.orange.shade700 : Colors.green.shade700))),
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _infoRow('Total PO', '\$${PurchaseService.fmtAmount(po.totalAmount)}'),
                _infoRow('Open to Invoice', '\$${PurchaseService.fmtAmount(openAmount)}',
                    valueColor: Colors.orange.shade700),
              ]),
            ),
          ),
          const SizedBox(height: 16),
          const Text('Items Pending Invoice', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          ...po.items.where((item) => item.receivedQuantity > item.invoicedQuantity).map((item) => Card(
            margin: const EdgeInsets.only(bottom: 8),
            child: Padding(
              padding: const EdgeInsets.all(12),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Expanded(child: Text('${item.itemSku} - ${item.itemName}',
                      style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 13))),
                  Text('\$${PurchaseService.fmtAmount(item.unitPrice)}/${item.unitOfMeasure}',
                      style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                ]),
                const SizedBox(height: 4),
                Row(children: [
                  Text('Received: ${_fmtQty(item.receivedQuantity)}',
                      style: TextStyle(fontSize: 12, color: Colors.teal.shade600)),
                  const SizedBox(width: 12),
                  Text('Invoiced: ${_fmtQty(item.invoicedQuantity)}',
                      style: TextStyle(fontSize: 12, color: Colors.blue.shade600)),
                  const Spacer(),
                  Text('Open: ${_fmtQty(item.receivedQuantity - item.invoicedQuantity)}',
                      style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600,
                          color: Colors.orange.shade700)),
                ]),
                const SizedBox(height: 4),
                Text('Open Amount: \$${PurchaseService.fmtAmount(_itemOpenAmt(item))}',
