...
class POListScreen extends StatefulWidget {
  ...
  const POListScreen({super.key, required this.authService, required this.purchaseService});
...
class _POListScreenState extends State<POListScreen> {
  ...
  static const _statuses = ['', 'DRAFT', 'CONFIRMED', 'RECEIVED', 'INVOICED', 'CANCELLED'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Purchase Orders'), actions: [
        IconButton(
          icon: const Icon(Icons.add_rounded),
          tooltip: 'New PO',
          onPressed: () async {
            await Navigator.push(context, MaterialPageRoute(
              builder: (_) => POFormScreen(authService: widget.authService, purchaseService: widget.purchaseService),
            ));
            _loadPOs();
          },
        ),
              ...
              children: _statuses.map((s) {
                ...
                return Padding(
                  ...
                  child: FilterChip(
                    ...
                    selectedColor: _statusColor(s.isEmpty ? '' : s).withValues(alpha: 0.15),
  ...
  Widget _buildContent() {
    ...
    if (_pos.isEmpty) return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              child: Padding(
                ...
                child: Row(children: [
                  Container(
                    ...
                    child: Center(child: Icon(Icons.description, color: _statusColor(po.status), size: 22)),
                  ),
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [
                      Text(po.poNumber, style: const TextStyle(fontWeight: FontWeight.w600, fontFamily: 'monospace')),
                      ...
                      Container(
                        ...
                        child: Text(po.status, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: _statusColor(po.status))),
                      ),
                    ]),
                    const SizedBox(height: 2),
                    Text(po.vendorName.isNotEmpty ? po.vendorName : po.vendorCode, style: TextStyle(fontSize: 13, color: Colors.grey.shade600)),
                    const SizedBox(height: 2),
                    Text('${po.currency} ${PurchaseService.fmtAmount(po.totalAmount)}', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.grey.shade700)),
                  ])),
