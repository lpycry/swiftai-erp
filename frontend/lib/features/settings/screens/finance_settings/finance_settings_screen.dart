...
class _PeriodsTabState extends State<_PeriodsTab> {
  ...
  Future<void> _togglePeriod(String id, bool newOpen) async {
    try {
      ...
      await _loadPeriods();
      _snack(
        newOpen ? 'Period opened' : 'Period closed',
  ...
  Future<void> _generatePeriods() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
              'Creates 12 monthly periods for the selected year and company.',
            ),
    ...
    try {
      final body = <String, dynamic>{
        'year': int.tryParse(yearCtrl.text) ?? _year,
      };
          ...
          child: Row(
            children: [
              DropdownButton<int>(
                ...
                items: List.generate(10, (i) => DateTime.now().year - 3 + i)
                    .map(
...
class _CoaTabState extends State<_CoaTab> {
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: [
                const DropdownMenuItem(
                  value: null,
                  child: Text('Global (use as default)'),
                ),
                ..._orgs.map(
                  (o) => DropdownMenuItem(
                    value: o['id'],
            ...
            if (_orgs.length > 1 && _selectedOrgId == null)
              Container(
                ...
                child: Text(
                  'Multiple companies detected. Select a company to generate per-company COA, or use Global for all.',
  ...
  Future<void> _executeInit(String coaType) async {
    ...
    try {
      ...
      await widget.glService.initializeCoa(
        coaType,
      ); // existing API — backend already supports org now
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                const Text(
                  'Initialize or reset the Chart of Accounts for a specific company code or globally.',
                  style: TextStyle(fontSize: 13),
                ),
                const SizedBox(height: 16),
                const Text(
                ...
                ...[
                  _standardRow(
                    'US GAAP',
                    'Standard for US-based companies (4-digit)',
                  ),
                  _standardRow(
                    'IFRS',
                  ...
                  _standardRow(
                    'China CAS',
                    'China Accounting Standards (4-4-2)',
                  ),
                ],
...
class _PaymentTermsTabState extends State<_PaymentTermsTab> {
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: TextField(
                      controller: discDaysCtrl,
                      decoration: const InputDecoration(
                        labelText: 'Discount Days',
                      ),
                      keyboardType: TextInputType.number,
                    ),
                  ),
  ...
  Future<void> _editPaymentTerm(String id, Map<String, dynamic> data) async {
    ...
    final discDaysCtrl = TextEditingController(
      text: '${data['discount_days'] ?? 0}',
    );
    final discPctCtrl = TextEditingController(
      text: '${data['discount_pct'] ?? 0}',
    );
  ...
  Future<void> _deletePaymentTerm(String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Delete this payment term? Standard terms cannot be deleted.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
...
class _PaymentTermCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final discount =
        data['discount_pct'] != null && (data['discount_pct'] as num) > 0;
    return Card(
      margin: const EdgeInsets.symmetric(vertical: 3, horizontal: 4),
      child: ListTile(
        leading: Container(
          ...
          height: 40,
          decoration: BoxDecoration(
            color: (isStandard ? AppTheme.accentBlue : AppTheme.accentGreen)
        ...
        subtitle: Text.rich(
          TextSpan(
            children: [
              ...
              if (discount)
                TextSpan(
                  text: ' · ${data['discount_pct']}% ${data['discount_days']}d',
                ),
              if (isStandard)
                TextSpan(
                  text: ' · Standard',
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: [
                  DropdownMenuItem(
                    value: 'E',
                    child: Text('🚪 E — Departure (EXW)'),
                  ),
                  DropdownMenuItem(
                    value: 'F',
                    child: Text('⛵ F — Main carriage unpaid (FCA, FAS, FOB)'),
                  ),
                  DropdownMenuItem(
                    value: 'C',
                    child: Text(
                  ...
                  DropdownMenuItem(
                    value: 'D',
                    child: Text('🏁 D — Arrival (DAP, DPU, DDP)'),
                  ),
                ],
...
const Map<String, String> _accountTypes = {
  ...
  'FGS': 'Finished Goods',
  'SFGS': 'Semi Finished Goods',
  'Other_Inv': 'Other Inventory',
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    'One account per type per company. Save again with the same company+type to update the GL account.',
                    style: TextStyle(
                      fontSize: 11,
        ...
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                Text(
                  '${item['org_code']} - ${item['org_name']}',
                  style: const TextStyle(fontWeight: FontWeight.w600),
                ),
                Text(
                  'Type: ${item['account_type']} — ${_accountTypes[item['account_type']] ?? ''}',
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              'Click "New" to assign a GL account type to a company code',
...
class _TaxRateDialogState extends State<_TaxRateDialog> {
  ...
  void initState() {
    ...
    if (widget.existing != null) {
      ...
      if (e['end_date'] != null) {
        _endDate = DateTime.parse(e['end_date'].toString());
      }
