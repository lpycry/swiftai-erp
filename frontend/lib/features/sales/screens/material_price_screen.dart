...
class _MaterialPriceListScreenState extends State<MaterialPriceListScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final uri = Uri.parse(
        'http://localhost:8080/api/v1/sales/material-prices',
  ...
  Future<void> _loadCustomers() async {
    try {
      ...
      if (resp.statusCode < 400) {
        _customers = ((jsonDecode(resp.body)['data'] as List?) ?? []);
      }
    } catch (_) {}
    if (mounted) {
  ...
  Future<void> _upsert(Map<String, dynamic>? existing) async {
    ...
    try {
      if (existing == null) {
        ...
        await widget.salesService.updateMaterialPrice(
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Add a sales price for a product',
...
class _MaterialPriceDialog extends StatefulWidget {
  ...
  @override
  State<_MaterialPriceDialog> createState() => _MaterialPriceDialogState();
}
  ...
  void initState() {
    ...
    if (widget.existing != null) {
      ...
      if (e['valid_from'] != null) {
        _validFrom = DateTime.parse(e['valid_from'].toString());
      }
      if (e['valid_to'] != null) {
        _validTo = DateTime.parse(e['valid_to'].toString());
      }
    }
  }
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(
                    value: 'PROMOTIONAL',
                    child: Text('Promotional', style: TextStyle(fontSize: 12)),
                  ),
                  DropdownMenuItem(
                    value: 'VOLUME',
                  ...
                  DropdownMenuItem(
                    value: 'CONTRACT',
                    child: Text('Contract', style: TextStyle(fontSize: 12)),
                  ),
                  DropdownMenuItem(
              ...
              DropdownButtonFormField<String>(
                ...
                items: [
                  const DropdownMenuItem(
                    value: '',
                    child: Text(
                      'All customers (standard price)',
                      style: TextStyle(fontSize: 12),
                    ),
                  ),
                  ...widget.customers.map<DropdownMenuItem<String>>(
                    (c) => DropdownMenuItem(
                      value: c['id']?.toString(),
                      child: Text(
