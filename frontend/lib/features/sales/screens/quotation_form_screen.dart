...
class _QuotationFormScreenState extends State<QuotationFormScreen> {
  ...
  void initState() {
    ...
    if (q != null) {
      ...
      if (q['valid_from'] != null) {
        _validFrom =
            DateTime.tryParse(q['valid_from'].toString()) ?? DateTime.now();
      }
      if (q['valid_to'] != null) {
        _validTo = DateTime.tryParse(q['valid_to'].toString());
      }
      ...
      for (final it in items) {
        _items.add(
          _LineItem(
            ...
            lineTotal: (it['line_total'] as num?)?.toDouble() ?? 0,
          ),
        );
      }
    }
  ...
  Future<void> _loadLookups() async {
    try {
      ...
      if (prodResp.statusCode < 400) {
        _products = ((jsonDecode(prodResp.body)['data'] as List?) ?? []);
      }
      final empResp = await http.get(
        Uri.parse('http://localhost:8080/api/v1/employees?mode=current'),
        headers: {'Authorization': 'Bearer $_token'},
      );
      if (empResp.statusCode < 400) {
        _employees = ((jsonDecode(empResp.body)['data'] as List?) ?? []);
      }
    } catch (_) {}
    if (mounted) setState(() {});
  }
  ...
  void _addItem() =>
      setState(() => _items.add(_LineItem(lineNo: _items.length + 1)));
  ...
  Future<void> _calculateTax() async {
    ...
    try {
      ...
      final resp = await http.post(
        Uri.parse(
          'http://localhost:8080/api/v1/sales/quotations/calculate-tax',
        ),
        headers: {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Tax: \$${taxAmount.toStringAsFixed(2)} at ${(taxRate * 100).toStringAsFixed(2)}% — $source',
            ),
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final data = <String, dynamic>{
        ...
        'items': _items.map((it) {
          final qty = double.tryParse(it.quantityCtrl.text) ?? it.quantity;
          final price = double.tryParse(it.unitPriceCtrl.text) ?? it.unitPrice;
          final disc = double.tryParse(it.discountCtrl.text) ?? it.discountPct;
      ...
      if (_validTo != null)
        data['valid_to'] =
            '${_validTo!.year}-${_validTo!.month.toString().padLeft(2, '0')}-${_validTo!.day.toString().padLeft(2, '0')}';
      final uri = isEdit
          ? Uri.parse(
              'http://localhost:8080/api/v1/sales/quotations/${widget.quotation!['id']}',
            )
          : Uri.parse('http://localhost:8080/api/v1/sales/quotations');
      final resp = isEdit
          ? await http.put(
              uri,
              headers: {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isEdit ? 'Quotation updated' : 'Quotation created'),
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      await Printing.sharePdf(
        ...
        filename: '${widget.quotation!['quotation_no']}.pdf',
      );
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      if (companyName.isNotEmpty) {
        ...
        if (companyAddress.isNotEmpty)
          sb.writeln('<div class="address">${companyAddress}</div>');
        sb.writeln('<hr style="border:1px solid #1565C0;margin:10px 0">');
      }
      sb.writeln('<h1>QUOTATION</h1>');
      sb.writeln(
        '<p><b>No:</b> ${q['quotation_no'] ?? ''} | <b>Date:</b> ${Fmt.dateStr(q['valid_from']?.toString())} | <b>Status:</b> ${q['status'] ?? ''}</p>',
      );
      sb.writeln(
        '<p><b>Customer:</b> ${q['customer_name'] ?? ''} (${q['customer_code'] ?? ''})</p>',
      );
      sb.writeln(
        '<table><tr><th>#</th><th>SKU</th><th>Description</th><th>Qty</th><th>UOM</th><th>Price</th><th>Disc%</th><th>Total</th></tr>',
      );
      for (var i = 0; i < items.length; i++) {
        final it = items[i] as Map<String, dynamic>;
        sb.writeln(
          '<tr><td>${i + 1}</td><td>${it['product_sku'] ?? ''}</td><td>${it['description'] ?? it['product_name'] ?? ''}</td><td class="right">${(it['quantity'] as num?)?.toStringAsFixed(2) ?? '0'}</td><td>${it['unit_of_measure'] ?? 'EA'}</td><td class="right">$currency ${(it['unit_price'] as num?)?.toStringAsFixed(2) ?? '0'}</td><td class="right">${(it['discount_pct'] as num?)?.toString() ?? '0'}%</td><td class="right">$currency ${(it['line_total'] as num?)?.toStringAsFixed(2) ?? '0'}</td></tr>',
        );
      }
      sb.writeln('</table>');
      sb.writeln(
        '<p class="total">Total: $currency ${(q['total_amount'] as num?)?.toStringAsFixed(2) ?? '0'}</p>',
      );
      if (((q['discount_pct'] as num?)?.toDouble() ?? 0) > 0)
        sb.writeln('<p>Discount: ${q['discount_pct']}%</p>');
      sb.writeln(
        '<p>Net: $currency ${(q['net_amount'] as num?)?.toStringAsFixed(2) ?? '0'}</p>',
      );
      sb.writeln(
        '<p>Tax: $currency ${(q['tax_amount'] as num?)?.toStringAsFixed(2) ?? '0'}</p>',
      );
      sb.writeln(
        '<p class="total">Grand Total: $currency ${(q['grand_total'] as num?)?.toStringAsFixed(2) ?? '0'}</p>',
      );
      if ((q['notes']?.toString() ?? '').isNotEmpty)
        sb.writeln('<p><b>Notes:</b> ${q['notes']}</p>');
      sb.writeln(
        '<p style="margin-top:40px">_________________________&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;_________________________</p>',
      );
      sb.writeln(
        '<p>Customer Signature&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Authorized Signature</p>',
      );
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Export Word error: $e'),
            backgroundColor: Colors.red,
          ),
        );
    }
              ...
              itemBuilder: (_) => [
                const PopupMenuItem(
                  ...
                  child: ListTile(
                    ...
                    title: Text('Export PDF', style: TextStyle(fontSize: 12)),
                    dense: true,
                ...
                const PopupMenuItem(
                  ...
                  child: ListTile(
                    ...
                    title: Text('Export Word', style: TextStyle(fontSize: 12)),
                    dense: true,
                    contentPadding: EdgeInsets.zero,
                  ),
                ),
            ...
            PopupMenuButton<String>(
              onSelected: (v) async {
                try {
                  await http.put(
                    Uri.parse(
                      'http://localhost:8080/api/v1/sales/quotations/${widget.quotation!['id']}/status',
                    ),
                    headers: {
                      'Content-Type': 'application/json',
                      'Authorization': 'Bearer $_token',
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            if (isEdit)
              Text(
                '${widget.quotation!['customer_code']} - ${widget.quotation!['customer_name']}',
            ...
            DropdownButtonFormField<String?>(
              ...
              decoration: const InputDecoration(
                labelText: 'Salesperson (Employee)',
                isDense: true,
              ...
              items: [
                ...
                ..._employees.map(
                  (e) => DropdownMenuItem(
                    ...
                    child: Text(
                      '${e['employee_code'] ?? ''} — ${e['full_name'] ?? ''} ${(e['position_title'] != null && (e['position_title'] as String).isNotEmpty) ? '(${e['position_title']})' : ''}',
                      style: const TextStyle(fontSize: 12),
                    ),
                  ),
                ),
              ...
              children: [
                ...
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    items: ['Net 30', 'Net 15', 'Net 60', 'COD']
                        .map((t) => DropdownMenuItem(value: t, child: Text(t)))
                        .toList(),
                    onChanged: (v) =>
                        setState(() => _paymentTerms = v ?? 'Net 30'),
                  ),
                ),
              ],
            ),
                ...
                child: Row(
                  children: [
                    Expanded(
                      flex: 3,
                      child: DropdownButtonFormField<String>(
                        value: e.value.productId.isEmpty
                            ? null
                            : e.value.productId,
                        isExpanded: true,
                        decoration: const InputDecoration(
                        ...
                        onChanged: (v) async {
                          ...
                          if (v != null && _customerId != null) {
                            try {
                              final resp = await http.get(
                                Uri.parse(
                                  'http://localhost:8080/api/v1/sales/material-prices/lookup?customer_id=$_customerId&product_id=$v',
                                ),
                                headers: {'Authorization': 'Bearer $_token'},
                              );
                              if (resp.statusCode < 400) {
                                ...
                                if (mp != null && mp['price'] != null) {
                                  ...
                                  final priceUnit =
                                      (mp['price_unit'] as num?)?.toInt() ?? 1;
                                  final unitPrice = price / priceUnit;
                                  setState(() {
                                    e.value.unitPriceCtrl.text = unitPrice
                                        .toStringAsFixed(2);
                                  });
                                  _recalc(e.value);
                                  return;
                    ...
                    Expanded(
                      flex: 1,
                      child: TextField(
                        controller: e.value.quantityCtrl,
                        ...
                        onChanged: (_) => _recalc(e.value),
                      ),
                    ),
                    Expanded(
                      flex: 2,
                      child: TextField(
                        controller: e.value.unitPriceCtrl,
                        keyboardType: TextInputType.number,
                        style: const TextStyle(fontSize: 11),
                        decoration: const InputDecoration(
                          isDense: true,
                    ...
                    Expanded(
                      flex: 1,
                      child: TextField(
                        controller: e.value.discountCtrl,
                        keyboardType: TextInputType.number,
                        textAlign: TextAlign.center,
                        style: const TextStyle(fontSize: 11),
                        decoration: const InputDecoration(
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _taxCalcRow(
                      'Tax Rate',
                      '${_taxCalcRate != null ? (_taxCalcRate! * 100).toStringAsFixed(3) : "-"}%',
                    ),
                    _taxCalcRow(
                      'Net Amount',
                    ...
                    _taxCalcRow(
                      'Tax Amount',
                      '\$${_taxCalcAmount?.toStringAsFixed(2) ?? "-"}',
                    ),
                    _taxCalcRow(
                      'Method',
                      _taxCalcSource?.replaceAll('_', ' ') ?? '-',
                    ),
            ...
            const SizedBox(height: 16),
            ElevatedButton(
              onPressed: (_saving || (!isEdit && _customerId == null))
                  ? null
                  : _save,
              child: _saving
                  ? const SizedBox(
                  ...
                  : Text(isEdit ? 'Save Changes' : 'Create Quotation'),
            ),
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: bold ? 16 : 12,
              fontWeight: bold ? FontWeight.w700 : FontWeight.w400,
              color: bold ? Colors.blue.shade800 : null,
            ),
          ),
        ],
      ),
