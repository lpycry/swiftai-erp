...
class _CustomerFormScreenState extends State<CustomerFormScreen> {
  ...
  late final TextEditingController _billStreetCtrl,
      _billCityCtrl,
      _billStateCtrl,
      _billZipCtrl,
      _billCountryCtrl;
  late final TextEditingController _shipStreetCtrl,
      _shipCityCtrl,
      _shipStateCtrl,
      _shipZipCtrl,
      _shipCountryCtrl;
  ...
  List<dynamic> _paymentTermsList = [];
  bool _loadingPaymentTerms = true;
  ...
  void initState() {
    ...
    _codeCtrl = TextEditingController(
      text: c?['customer_code']?.toString() ?? '',
    );
    _nameCtrl = TextEditingController(text: c?['name']?.toString() ?? '');
    _taxNumberCtrl = TextEditingController(
      text: c?['tax_number']?.toString() ?? '',
    );
    _contactPersonCtrl = TextEditingController(
      text: c?['contact_person']?.toString() ?? '',
    );
    _emailCtrl = TextEditingController(
      text: c?['contact_email']?.toString() ?? '',
    );
    _phoneCtrl = TextEditingController(
      text: c?['contact_phone']?.toString() ?? '',
    );
    _paymentTerms = c?['payment_terms']?.toString();
    _exemptionCertCtrl = TextEditingController(
      text: c?['tax_exemption_cert']?.toString() ?? '',
    );
    _exemptReasonCtrl = TextEditingController(
      text: c?['tax_exempt_reason']?.toString() ?? '',
    );
    _billStreetCtrl = TextEditingController(
      text: c?['billing_street']?.toString() ?? '',
    );
    _billCityCtrl = TextEditingController(
      text: c?['billing_city']?.toString() ?? '',
    );
    _billStateCtrl = TextEditingController(
      text: c?['billing_state']?.toString() ?? '',
    );
    _billZipCtrl = TextEditingController(
      text: c?['billing_zip']?.toString() ?? '',
    );
    _billCountryCtrl = TextEditingController(
      text: c?['billing_country']?.toString() ?? 'US',
    );
    _shipStreetCtrl = TextEditingController(
      text: c?['shipping_street']?.toString() ?? '',
    );
    _shipCityCtrl = TextEditingController(
      text: c?['shipping_city']?.toString() ?? '',
    );
    _shipStateCtrl = TextEditingController(
      text: c?['shipping_state']?.toString() ?? '',
    );
    _shipZipCtrl = TextEditingController(
      text: c?['shipping_zip']?.toString() ?? '',
    );
    _shipCountryCtrl = TextEditingController(
      text: c?['shipping_country']?.toString() ?? 'US',
    );
    ...
    if (startStr != null && startStr.isNotEmpty) {
      _exemptStartDate = DateTime.tryParse(startStr) ?? DateTime.now();
    }
    final endStr = c?['tax_exempt_end_date']?.toString();
    if (endStr != null && endStr.isNotEmpty) {
      _exemptEndDate = DateTime.tryParse(endStr);
    }
    _defaultTaxJurisdictionId =
        c?['default_tax_jurisdiction_id']?.toString() ?? '';
    if (c != null && c['certificates'] != null) {
      _certificates = c['certificates'] as List<dynamic>;
    }
  ...
  void dispose() {
    ...
    _emailCtrl.dispose();
    _phoneCtrl.dispose();
    _exemptionCertCtrl.dispose();
    _exemptReasonCtrl.dispose();
  ...
  Future<void> _loadPaymentTerms() async {
    try {
      ...
      if (resp.statusCode < 400) {
        _paymentTermsList = ((jsonDecode(resp.body)['data'] as List?) ?? []);
        final codes = _paymentTermsList
            .map((pt) => pt['code']?.toString() ?? '')
            .where((v) => v.isNotEmpty)
            .toList();
        if ((_paymentTerms == null ||
                _paymentTerms!.isEmpty ||
                !codes.contains(_paymentTerms)) &&
            codes.isNotEmpty) {
          _paymentTerms = codes.first;
        }
      }
    } catch (_) {}
    if (mounted) setState(() => _loadingPaymentTerms = false);
  }
  ...
  Future<void> _uploadCertificate(String certType) async {
    if (_customerId.isEmpty) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Save the customer first, then upload certificates.'),
    ...
    try {
      ...
      final uri = Uri.parse(
        'http://localhost:8080/api/v1/sales/customers/$_customerId/certificates',
      );
  ...
  Future<void> _loadCertificates() async {
    ...
    try {
      ...
      if (resp.statusCode < 400) {
        setState(
          () =>
              _certificates = ((jsonDecode(resp.body)['data'] as List?) ?? []),
        );
      }
    } catch (_) {}
  }
  ...
  Future<void> _deleteCertificate(String certId) async {
    try {
      final resp = await http.delete(
        Uri.parse(
          'http://localhost:8080/api/v1/sales/customers/$_customerId/certificates/$certId',
        ),
        headers: {'Authorization': 'Bearer $_token'},
      );
      if (resp.statusCode >= 400) {
  ...
  Future<void> _save() async {
    ...
    try {
      final data = <String, dynamic>{
        'customer_code': _codeCtrl.text.trim(),
        ...
        'billing_street': _billStreetCtrl.text.trim(),
        'billing_city': _billCityCtrl.text.trim(),
        'billing_state': _billStateCtrl.text.trim(),
        'billing_zip': _billZipCtrl.text.trim(),
        'billing_country': _billCountryCtrl.text.trim(),
        'shipping_street': _shipStreetCtrl.text.trim(),
        'shipping_city': _shipCityCtrl.text.trim(),
        'shipping_state': _shipStateCtrl.text.trim(),
        'shipping_zip': _shipZipCtrl.text.trim(),
        'shipping_country': _shipCountryCtrl.text.trim(),
        'status': _status,
        'is_tax_exempt': _isTaxExempt,
        'default_tax_jurisdiction_id': _defaultTaxJurisdictionId.isEmpty
            ? null
            : _defaultTaxJurisdictionId,
      };
      ...
      if (_isTaxExempt) {
        ...
        data['tax_exempt_start_date'] =
            '${_exemptStartDate.year}-${_exemptStartDate.month.toString().padLeft(2, '0')}-${_exemptStartDate.day.toString().padLeft(2, '0')}';
  ...
  Future<void> _deleteCustomer() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Delete "${widget.customer!['name']}" (${widget.customer!['customer_code']})?',
        ),
        actions: [
          TextButton(
          ...
          bottom: const TabBar(
            ...
            tabs: [
              Tab(
                ...
                child: Text('General', style: TextStyle(fontSize: 12)),
              ),
              Tab(
                icon: Icon(Icons.receipt_long_outlined, size: 16),
                child: Text('Tax Exemption', style: TextStyle(fontSize: 12)),
              ),
            ],
          ),
        ),
  ...
  Widget _buildBasicFields() => Column(
    ...
    children: [
      ...
      _sectionHeader('Tax & Classification'),
      const SizedBox(height: 12),
      Row(
        children: [
          Expanded(
            child: TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Tax Number / EIN'),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
      ...
      const SizedBox(height: 16),
      _sectionHeader('Payment & Currency'),
      const SizedBox(height: 12),
      Row(
        children: [
          Expanded(
            ...
            child: DropdownButtonFormField<String>(
              value:
                  _paymentTermsList.any(
                    (pt) => pt['code']?.toString() == _paymentTerms,
                  )
                  ? _paymentTerms
                  : null,
              decoration: const InputDecoration(labelText: 'Payment Terms'),
              isExpanded: true,
              items: _paymentTermsList.map((pt) {
                final code = pt['code']?.toString() ?? '';
              ...
              onChanged: _loadingPaymentTerms
                  ? null
  ...
  Widget _buildTaxFields() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      _sectionHeader('Tax Exemption'),
      const SizedBox(height: 12),
      Container(
            ...
            if (_isTaxExempt) ...[
              ...
              Row(
                children: [
                  Expanded(
                    child: TextFormField(
                      controller: _exemptionCertCtrl,
                      decoration: const InputDecoration(
                        labelText: 'Exemption Certificate #',
                        isDense: true,
                        hintText: 'e.g. Resale Cert #12345',
                      ),
                  ...
                  Expanded(
                    child: TextFormField(
                      controller: _exemptReasonCtrl,
                      decoration: const InputDecoration(
                        labelText: 'Exemption Reason',
                        isDense: true,
                        hintText: 'RESALE, GOV, NON_PROFIT',
                      ),
                      style: const TextStyle(fontSize: 12),
                    ),
                  ),
                    ...
                    child: InkWell(
                      onTap: () async {
                        final d = await showDatePicker(
                          context: context,
                          initialDate: _exemptStartDate,
                          firstDate: DateTime(2020),
                          lastDate: DateTime(2035),
                        );
                        if (d != null) setState(() => _exemptStartDate = d);
                      },
                  ...
                  Expanded(
                    child: InkWell(
                      onTap: () async {
                        ...
                        if (d != null) setState(() => _exemptEndDate = d);
                      },
                      child: InputDecorator(
                        decoration: const InputDecoration(
                          labelText: 'End Date',
      ...
      if (!_loadingJurisdictions) ...[
        DropdownButtonFormField<String>(
          value: _defaultTaxJurisdictionId.isEmpty
              ? ''
              : _defaultTaxJurisdictionId,
          decoration: const InputDecoration(
            labelText: 'Default Tax Jurisdiction',
            hintText: 'Optional',
            isDense: true,
          ),
          isExpanded: true,
  ...
  Widget _buildCertFields() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      _sectionHeader('Exemption Certificates'),
      const SizedBox(height: 12),
      if (_customerId.isNotEmpty)
        ...['TAX_EXEMPT', 'RESALE', 'OTHER'].map(
          (ct) => Padding(
            padding: const EdgeInsets.only(bottom: 8),
            child: OutlinedButton.icon(
              icon: const Icon(Icons.upload_file, size: 18),
              label: Text('Upload $ct Certificate'),
              onPressed: () => _uploadCertificate(ct),
            ),
          ),
        ),
      const SizedBox(height: 16),
        ...
        ..._certificates.map(
          (cert) => Card(
            ...
            child: ListTile(
              ...
              title: Text(
                cert['file_name']?.toString() ?? '',
                style: const TextStyle(fontSize: 13),
              ),
              subtitle: Text(
                '${cert['cert_type']} · ${cert['file_size']} bytes',
              ...
              trailing: IconButton(
                ...
                onPressed: () => _deleteCertificate(cert['id'].toString()),
              ),
            ),
          ),
        ),
        ...
        Container(
          ...
          child: const Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Save the customer first, then upload certificates.',
