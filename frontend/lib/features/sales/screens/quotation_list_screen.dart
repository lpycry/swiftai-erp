...
class QuotationListScreen extends StatefulWidget {
  ...
  const QuotationListScreen({super.key, required this.authService, required this.salesService});
  @override State<QuotationListScreen> createState() => _QuotationListScreenState();
}
...
class _QuotationListScreenState extends State<QuotationListScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final uri = Uri.parse('http://localhost:8080/api/v1/sales/quotations').replace(queryParameters: params);
  ...
  Future<void> _printQuotation(Map<String, dynamic> q) async {
    try {
      final resp = await http.get(Uri.parse('http://localhost:8080/api/v1/sales/quotations/${q['id']}'), headers: {'Authorization': 'Bearer $_token'});
  ...
  Future<void> _exportPdf(Map<String, dynamic> q) async {
    try {
      ...
      final data = jsonDecode(resp.body)['data'] as Map<String, dynamic>;
      final pdf = await _buildPdf(data);
      await Printing.sharePdf(bytes: await pdf.save(), filename: '${q['quotation_no']}.pdf');
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$e'), backgroundColor: Colors.red));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          DropdownButtonHideUnderline(child: DropdownButton<String>(
            value: _statusFilter, hint: const Text('Status', style: TextStyle(fontSize: 12)), isDense: true, padding: const EdgeInsets.symmetric(horizontal: 8),
            items: const [
              ...
              DropdownMenuItem(value: 'ACCEPTED', child: Text('Accepted', style: TextStyle(fontSize: 12))),
          ...
          IconButton(icon: const Icon(Icons.add_rounded), onPressed: () async {
            await Navigator.push(context, MaterialPageRoute(builder: (_) => QuotationFormScreen(authService: widget.authService, salesService: widget.salesService)));
            _load();
          }),
        ],
      ),
      body: _loading ? const Center(child: CircularProgressIndicator())
        : _items.isEmpty
          ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.description_outlined, size: 48, color: Colors.grey.shade400), const SizedBox(height: 12), const Text('No quotations')]))
              ...
              itemBuilder: (_, i) {
                ...
                return Card(margin: const EdgeInsets.symmetric(vertical: 4), child: InkWell(borderRadius: BorderRadius.circular(12),
                  onTap: () async {
                    ...
                    try {
                      final resp = await http.get(Uri.parse('http://localhost:8080/api/v1/sales/quotations/${e['id']}'), headers: {'Authorization': 'Bearer $_token'});
                      if (resp.statusCode < 400) {
                        final full = jsonDecode(resp.body)['data'] as Map<String, dynamic>;
                        await Navigator.push(context, MaterialPageRoute(builder: (_) => QuotationFormScreen(authService: widget.authService, salesService: widget.salesService, quotation: full)));
                      } else {
                        await Navigator.push(context, MaterialPageRoute(builder: (_) => QuotationFormScreen(authService: widget.authService, salesService: widget.salesService, quotation: e)));
                      }
                    } catch (_) {
                      await Navigator.push(context, MaterialPageRoute(builder: (_) => QuotationFormScreen(authService: widget.authService, salesService: widget.salesService, quotation: e)));
                    }
                  ...
                  child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
                    Container(width: 6, height: 50, decoration: BoxDecoration(color: _statusColor(status), borderRadius: BorderRadius.circular(3))),
                    const SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        Text(e['quotation_no']?.toString() ?? '', style: TextStyle(fontSize: 13, fontFamily: 'monospace', fontWeight: FontWeight.w600)),
                      ...
                      Text('${e['customer_code'] ?? ''} - ${e['customer_name'] ?? ''}', style: const TextStyle(fontSize: 13)),
                      const SizedBox(height: 2),
                      Row(children: [
                        Text('\$${(e['grand_total'] as num?)?.toStringAsFixed(2) ?? '0.00'}', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: Colors.teal.shade700)),
                        const Spacer(),
                        IconButton(icon: Icon(Icons.picture_as_pdf, size: 16, color: Colors.red.shade400), onPressed: () => _printQuotation(e), padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 28, minHeight: 28), tooltip: 'Print / Preview'),
                        IconButton(icon: Icon(Icons.file_download, size: 16, color: Colors.blue.shade400), onPressed: () => _exportPdf(e), padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 28, minHeight: 28), tooltip: 'Export PDF'),
                      ]),
                    ])),
                  ])),
                ));
