...
class _OutstandingInvoicesScreenState extends State<OutstandingInvoicesScreen> {
  ...
  Future<void> _loadFilters() async {
    ...
    try {
      ...
      if (vr.statusCode < 400)
        _vendors = ((jsonDecode(vr.body)['data'] ?? []) as List)
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final uri = Uri.parse(
        'http://localhost:8080/api/v1/purchase/outstanding-invoices',
      );
      ...
      if (resp.statusCode < 400) {
        ...
        _invoices = data
            .map((d) => _InvoiceRow.fromJson(d as Map<String, dynamic>))
            .toList();
      }
    } catch (_) {}
          ...
          Container(
            ...
            child: Column(
              children: [
                  ...
                  children: [
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        onChanged: (v) {
                          setState(() => _vendorFilter = v == '' ? null : v);
                          _load();
                        },
                      ),
                    ),
                    ...
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        onChanged: (v) {
                          setState(() => _productFilter = v == '' ? null : v);
                          _load();
                        },
                      ),
                    ),
                ...
                Row(
                  children: [
                    _dateField('From', _dateFrom, (d) {
                      setState(() => _dateFrom = d);
                      _load();
                    }),
                    const SizedBox(width: 8),
                    _dateField('To', _dateTo, (d) {
                      setState(() => _dateTo = d);
                      _load();
                    }),
                    const Spacer(),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                Text(
                  'Outstanding: \$${ApService.fmtAmount(totalOpen)}',
                  style: TextStyle(
                    fontSize: 12,
            ...
            child: _loading
                ...
                : RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) => _buildRow(_invoices[i]),
                    ),
                  ),
          ),
        ],
  ...
  DropdownMenuItem<String> _dd(String value, String label) => DropdownMenuItem(
    value: value,
        ...
        child: Row(
          children: [
            ...
            Expanded(
              flex: 2,
              child: Text(
                inv.vendorName.isNotEmpty ? inv.vendorName : inv.vendorCode,
            ...
            Expanded(
              flex: 1,
              child: Text(
                '\$${ApService.fmtAmount(inv.openAmount)}',
                textAlign: TextAlign.right,
                style: TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w600,
...
class _InvoiceDetail extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = invoice.daysOverdue > 0
        ? Colors.red
        : invoice.daysOverdue > -5
        ? Colors.orange
        : Colors.green;
    return Scaffold(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _row('Invoice Date', Fmt.dateStr(invoice.invoiceDate)),
                  _row('Due Date', Fmt.dateStr(invoice.dueDate)),
                  _row(
                    'Status',
                    invoice.daysOverdue > 0
                        ? '${invoice.daysOverdue}d overdue'
                        : '${-invoice.daysOverdue}d remaining',
                    color,
                  ),
                  const Divider(),
                  _row(
                    'Total Amount',
                    '\$${ApService.fmtAmount(invoice.totalAmount)}',
                    null,
                    true,
                  ),
                  _row(
                    'Paid',
                    '\$${ApService.fmtAmount(invoice.paidAmount)}',
                    Colors.green,
                  ),
                  _row(
                    'Open Balance',
                    '\$${ApService.fmtAmount(invoice.openAmount)}',
                    color,
                    true,
                  ),
                  if (invoice.poNumber.isNotEmpty)
                    _row('PO Reference', invoice.poNumber),
                ],
              ),
            ),
          ),
              ...
              onPressed: () => Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) => VendorPaymentScreen(
                    ...
                    apService: apService,
                    preselectedVendorId: invoice.vendorId,
                  ),
                ),
              ),
            ),
