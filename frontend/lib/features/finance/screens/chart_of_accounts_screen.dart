...
class _ChartOfAccountsScreenState extends State<ChartOfAccountsScreen> {
  ...
  List<AccountModel> _flatten(List<AccountModel> list) {
    ...
    for (final n in list) {
      r.add(n);
      if (n.children != null) r.addAll(_flatten(n.children!));
    }
    return r;
  }
  ...
  void _search(String q) {
    ...
    widget.glService
        .searchAccounts(q.trim())
        .then((r) {
          if (mounted) setState(() => _searchResults = r);
        })
        .catchError((e) {
          if (mounted) _err('Search failed: $e');
        });
  }
            ...
            child: Column(
              ...
              children: [
                ...
                SwitchListTile(
                  ...
                  subtitle: Text(
                    openItemManaged
                        ? 'Line items require clearing'
                        : 'Posted lines are not open-item managed',
                    style: TextStyle(
                      fontSize: 12,
                      color: AppTheme.textSecondary,
                    ),
                  ),
                  value: openItemManaged,
                  onChanged: isLeaf
                      ? (v) => setS(() => openItemManaged = v)
                      : null,
                ),
                const SizedBox(height: 12),
                if (selParent == null)
                ...
                SwitchListTile(
                  ...
                  subtitle: Text(
                    isLeaf
                        ? 'Postable (can post entries)'
                        : 'Grouping account only',
                    style: TextStyle(
                      fontSize: 12,
                      color: AppTheme.textSecondary,
                    ),
              ...
              onPressed: () async {
                ...
                try {
                  final data = <String, dynamic>{
                    'account_code': codeCtrl.text,
                    'account_name': nameCtrl.text,
                    'account_type': type,
                    'is_leaf': isLeaf,
                    'reconciliation_type': recType,
                    'open_item_managed': isLeaf && openItemManaged,
                    'description': descCtrl.text,
                  };
                  if (selParent != null && selParent!.isNotEmpty)
                    data['parent_id'] = selParent;
  ...
  void _edit(AccountModel a) {
    ...
    bool openItemManaged = a.openItemManaged;
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: AccountModel.typeColor(
                      a.type,
                    ).withValues(alpha: 0.1),
                    borderRadius: BorderRadius.circular(4),
                  ),
                  child: Text(
                    '${a.code}  ${AccountModel.typeDisplayName(a.type)}',
                    style: TextStyle(
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: [
                    ...
                    ..._flat
                        .where((x) => x.id != a.id && x.isActive && !x.isLeaf)
                        .map(
                          (x) => DropdownMenuItem(
                            value: x.id,
                            child: Text(
  ...
  Future<void> _delete(AccountModel a) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to delete "${a.code} ${a.name}"?\n\n'
  ...
  Future<void> _reactivate(AccountModel a) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Reactivate "${a.code} ${a.name}"?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: const Text('Cancel'),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Account reactivated'),
            backgroundColor: AppTheme.successColor,
          ),
        );
      }
            ...
            child: Row(
              children: [
                ...
                Container(
                  ...
                  height: 38,
                  decoration: BoxDecoration(
                    color: AppTheme.accentGradientStart.withValues(alpha: 0.1),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: IconButton(
                    icon: const Icon(
          ...
          Expanded(
            child: _loading
                ...
                : ListView(
                    ...
                    children: _roots.map((a) => _node(a, 0)).toList(),
                  ),
          ),
        ],
      ),
          ...
          child: Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  AccountModel.typeDisplayName(a.type),
                  style: TextStyle(
                    fontSize: 10,
                    color: c,
                    fontWeight: FontWeight.w600,
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    '${AccountModel.typeDisplayName(a.type)}  ·  ${a.isLeaf ? 'Postable' : 'Grouping'}  ·  ${a.isActive ? 'Active' : 'Inactive'}',
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.pop(ctx);
                _create(parentId: a.id, parentCode: a.code);
              },
            ),
            ListTile(
              leading: Icon(
                Icons.delete_outlined,
                color: a.isActive ? AppTheme.errorColor : AppTheme.textMuted,
                size: 20,
              ),
              title: Text(
                a.isActive ? 'Delete Account' : 'Account Inactive',
                style: TextStyle(
                  fontSize: 14,
                  color: a.isActive ? AppTheme.textPrimary : AppTheme.textMuted,
                ),
              ),
              enabled: a.isActive,
              onTap: a.isActive
