...
class MovementListScreen extends StatefulWidget {
  ...
  const MovementListScreen({super.key, required this.authService, required this.warehouseService});
  @override State<MovementListScreen> createState() => _MovementListScreenState();
}
...
class _MovementListScreenState extends State<MovementListScreen> {
  ...
  @override void initState() { super.initState(); _loadWarehouses(); _load(); }
  ...
  Future<void> _onWhChanged(String? whId) async {
    ...
    if (whId != null) {
      try {
        final uri = Uri.parse('http://localhost:8080/api/v1/warehouse/bins?warehouse_id=$whId');
    ...
    return AppLayout(
      ...
      body: Column(children: [
            ...
            Row(children: [
              Expanded(
                child: DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration(
                    labelText: 'Warehouse', isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 6),
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: BorderSide(color: Colors.grey.shade300)),
                  ),
                  isExpanded: true,
                  items: [
                    const DropdownMenuItem(value: null, child: Text('All', style: TextStyle(fontSize: 11))),
              ...
              Expanded(
                child: DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration(
                    labelText: 'Bin Location', isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 6),
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: BorderSide(color: Colors.grey.shade300)),
                  ),
                  isExpanded: true,
                  items: [
            ...
            Row(children: [
              Expanded(
                child: InkWell(
                  ...
                  child: InputDecorator(
                    decoration: InputDecoration(
                      labelText: 'Date From', isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 6),
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: BorderSide(color: Colors.grey.shade300)),
              ...
              Expanded(
                child: InkWell(
                  ...
                  child: InputDecorator(
                    decoration: InputDecoration(
                      labelText: 'Date To', isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 6),
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: BorderSide(color: Colors.grey.shade300)),
                      suffixIcon: Icon(Icons.date_range, size: 16, color: Colors.grey.shade500),
                    ),
        ...
        Container(
          ...
          child: Row(children: [
            Text('${_movements.length} movement(s)',
        ...
        if (_movements.isNotEmpty)
          Container(
            ...
            child: Row(children: [
              ...
              Expanded(flex: 2, child: Text('From', textAlign: TextAlign.right, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 10))),
              Expanded(flex: 2, child: Text('To', textAlign: TextAlign.right, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 10))),
            ]),
          ),
        // ── List ──
        Expanded(
          child: _loading
              ...
              : _movements.isEmpty
                  ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Text('No movements', style: TextStyle(color: Colors.grey.shade500)),
                    ]))
                  : RefreshIndicator(
                      onRefresh: _load,
...
class _MovementRow extends StatelessWidget {
  ...
  Color _typeColor(String t) {
    switch (t) {
      case 'goods_receipt': return Colors.green;
      case 'goods_issue': return Colors.red;
  ...
  @override
  Widget build(BuildContext context) {
    final createdAt = (m['created_at']?.toString() ?? '').length >= 10
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(flex: 2, child: Text(createdAt, style: TextStyle(fontSize: 10, color: Colors.grey.shade600))),
          Expanded(flex: 2, child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1),
            decoration: BoxDecoration(color: color.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(3)),
            child: Text(_typeLabel(txType), style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: color)),
          )),
          Expanded(flex: 2, child: Text(sku, style: const TextStyle(fontSize: 11, fontFamily: 'monospace'), overflow: TextOverflow.ellipsis)),
          Expanded(flex: 1, child: Text(qty.toStringAsFixed(0), textAlign: TextAlign.right,
              style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: qty > 0 ? Colors.green.shade700 : Colors.red.shade700))),
          Expanded(flex: 2, child: Text(from, textAlign: TextAlign.right,
              style: TextStyle(fontSize: 9, color: Colors.grey.shade600), overflow: TextOverflow.ellipsis)),
          Expanded(flex: 2, child: Text(to.isNotEmpty ? to : '-', textAlign: TextAlign.right,
              style: TextStyle(fontSize: 9, color: Colors.grey.shade600), overflow: TextOverflow.ellipsis)),
        ],
      ),
    );
