...
class _EmployeeDetailScreenState extends State<EmployeeDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final detail = await widget.employeeService.getEmployeeDetail(
        widget.employeeId,
      );
      final history = await widget.employeeService.getDataHistory(
        widget.employeeId,
      );
      setState(() {
        _base = detail['base'];
                  ...
                  TextField(
                    ...
                    onChanged: (v) {
                      try {
                        ...
                        if (j is Map) {
                          payload.clear();
                          payload.addAll(j as Map<String, dynamic>);
                        }
                      } catch (_) {}
                    },
                  ),
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => [
              const PopupMenuItem(
                ...
                child: ListTile(
                  ...
                  title: Text('Position Assignment'),
                  dense: true,
              ...
              const PopupMenuItem(
                value: 'SALARY',
                child: ListTile(
                  leading: Icon(Icons.attach_money, size: 18),
                  title: Text('Salary'),
                  dense: true,
                  contentPadding: EdgeInsets.zero,
                ),
              ...
              const PopupMenuItem(
                value: 'CONTACT',
                child: ListTile(
                  leading: Icon(Icons.contact_phone_outlined, size: 18),
                  title: Text('Contact'),
                  dense: true,
                  contentPadding: EdgeInsets.zero,
                ),
            ...
            child: const Padding(
              ...
              child: Row(
                children: [Icon(Icons.add, size: 18), Text('Info Type')],
              ),
            ),
          ),
        ],
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  // Infotype timeline
                  Text(
                    'Data History (Infotype Timeline)',
                    style: TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w600,
                      color: Colors.grey.shade700,
              ...
              trailing: PopupMenuButton<String>(
                onSelected: (v) async {
                  if (v == 'delete') {
                    ...
                    if (ok == true) {
                      try {
                        await widget.employeeService.deleteDataHistory(
                          rec['record_id'],
                        );
                        _load();
                        _msg('Record deleted');
  ...
  String _payloadSummary(String code, Map<String, dynamic> p) {
    switch (code) {
      case 'POS_ASSIGN':
        return '${p['position_code'] ?? p['position_id'] ?? ''} — ${p['job_title'] ?? ''}';
      case 'SALARY':
        return '${p['salary_amount'] ?? ''} ${p['salary_currency'] ?? ''} / ${p['pay_frequency'] ?? ''}';
      case 'ADDRESS':
        return '${p['street'] ?? ''}, ${p['city'] ?? ''}, ${p['country'] ?? ''}';
      case 'CONTACT':
  ...
  void _submit() async {
    ...
    try {
      await widget.onSave({
        ...
        'emergency_contacts': _emerNameCtrl.text.trim().isEmpty
            ? '[]'
            : jsonEncode([
                {
                  'name': _emerNameCtrl.text.trim(),
                  'phone': _emerPhoneCtrl.text.trim(),
                  'relation': _emerRelationCtrl.text.trim(),
                },
              ]),
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  style: const TextStyle(fontSize: 13),
                  validator: (v) =>
                      v != null && v.isNotEmpty && !_emailRegex.hasMatch(v)
                      ? 'Invalid email'
                      : null,
                ),
                const SizedBox(height: 10),
                  ...
                  children: [
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        items: ['Regular', 'Part Time', 'Contractor', 'Intern']
                            .map(
                              (t) => DropdownMenuItem(
                                value: t,
                                child: Text(
                            ...
                            .toList(),
                        onChanged: (v) =>
                            setState(() => _workerType = v ?? 'Regular'),
                      ),
                    ),
                    const Spacer(),
                    CheckboxListTile(
...
class _PosAssignFormState extends State<_PosAssignForm> {
  ...
  Future<void> _loadRefs() async {
    try {
      _positions = await PositionService(
            ...
            children: [
              DropdownButtonFormField<dynamic>(
                ...
                items: _positions
                    ...
                    .toList(),
                onChanged: (v) {
                  widget.payload['position_id'] = v.toString();
                },
              ),
              const SizedBox(height: 10),
              TextField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Job Title (override)',
                  hintText: 'Optional',
                ),
                style: const TextStyle(fontSize: 13),
                onChanged: (v) => widget.payload['job_title'] = v,
              ),
            ],
          );
  }
          ...
          children: [
            ...
            Expanded(
              child: TextField(
                ...
                style: const TextStyle(fontSize: 13),
                onChanged: (v) => payload['salary_currency'] = v,
              ),
            ),
            const SizedBox(width: 10),
            Expanded(
              child: TextField(
                ...
                style: const TextStyle(fontSize: 13),
                onChanged: (v) => payload['pay_frequency'] = v,
              ),
            ),
          ],
        ),
...
class _AddressForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        TextField(
          decoration: const InputDecoration(labelText: 'Street', isDense: true),
          style: const TextStyle(fontSize: 13),
          onChanged: (v) => payload['street'] = v,
        ),
        const SizedBox(height: 8),
        Row(
          children: [
    ...
    return Column(
      ...
      children: [
        ...
        TextField(
          ...
          style: const TextStyle(fontSize: 13),
          onChanged: (v) => payload['phone'] = v,
        ),
        const SizedBox(height: 8),
        TextField(
          decoration: const InputDecoration(labelText: 'Mobile', isDense: true),
          keyboardType: TextInputType.phone,
          style: const TextStyle(fontSize: 13),
          onChanged: (v) => payload['mobile'] = v,
        ),
      ],
    );
  }
