...
func (r *ProductRepo) CreateZone(ctx context.Context, req *whmodels.CreateZoneRequest) (*whmodels.WarehouseZone, error) {
	...
	if z.ZoneType == "" { z.ZoneType = "storage" }
	...
	if err != nil { return nil, fmt.Errorf("create zone: %w", err) }
	return z, nil
}
...
func (r *ProductRepo) GetBin(ctx context.Context, id uuid.UUID) (*whmodels.WarehouseBin, error) {
	...
	if err != nil { return nil, err }
	if len(list) == 0 { return nil, fmt.Errorf("bin not found") }
	return list[0], nil
}
...
func (r *ProductRepo) CreateBin(ctx context.Context, req *whmodels.CreateBinRequest) (*whmodels.WarehouseBin, error) {
	...
	if err != nil { return nil, fmt.Errorf("create bin: %w", err) }
	return b, nil
}
...
func (r *ProductRepo) ListBarcodes(ctx context.Context, productID uuid.UUID) ([]*whmodels.ProductBarcode, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&b.ID, &b.ProductID, &b.Barcode, &b.BarcodeType, &b.IsPrimary, &b.CreatedAt); err != nil { return nil, err }
		list = append(list, b)
	}
	return list, nil
}
...
func (r *ProductRepo) CreateBarcode(ctx context.Context, productID uuid.UUID, barcode, barcodeType string) (*whmodels.ProductBarcode, error) {
	...
	if b.BarcodeType == "" { b.BarcodeType = "EAN-13" }
	...
	if err != nil { return nil, fmt.Errorf("create barcode: %w", err) }
	return b, nil
}
...
func (r *ProductRepo) ListPhotos(ctx context.Context, productID uuid.UUID) ([]*whmodels.ProductPhoto, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.ProductID, &p.IsPrimary, &p.SortOrder, &p.FilePath, &p.FileName, &p.FileSize, &p.MimeType, &p.AITags, &p.CreatedAt); err != nil { return nil, err }
		list = append(list, p)
	}
	return list, nil
}
...
func (r *ProductRepo) CreatePhoto(ctx context.Context, productID uuid.UUID, fileName, filePath string, fileSize int, mimeType string) (*whmodels.ProductPhoto, error) {
	...
	if err != nil { return nil, fmt.Errorf("create photo: %w", err) }
	return p, nil
}
...
func (r *WarehouseRepo) ShipOutbound(ctx context.Context, id, tenantID, userID uuid.UUID) error {
    ...
    if err != nil { return fmt.Errorf("get order: %w", err) }
    ...
    lineRows, err := tx.Query(ctx, `
        ...
        FROM outbound_order_lines WHERE order_id = $1 ORDER BY id
    `, id)
    if err != nil { return fmt.Errorf("query lines: %w", err) }
    defer lineRows.Close()
    ...
    for _, line := range lines {
        ...
        _, err = tx.Exec(ctx, `UPDATE outbound_order_lines SET shipped_qty = $1 WHERE id = $2`,
            shipQty, line.ID)
        if err != nil { return fmt.Errorf("update line: %w", err) }
        ...
        `, movementID, tenantID, "goods_issue",
            ...
            "posted", userID, now, now, userID)
        if err != nil { return fmt.Errorf("insert movement: %w", err) }
        ...
        if err != nil { return fmt.Errorf("update stock: %w", err) }
    }
